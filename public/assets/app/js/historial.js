var SnippetHistory = function() {
                    ...
                    console.log(rpta) 
                      if (rpta.status == 200) {
                          img_update.processQueue();    
                             window.location.replace("/historial/detalle/" + $('#next').val());

                        } else {
                            notify('Hubo un error al crear el historial', 'la la-warning', 'danger')
                        }
                ...
                form.validate({
                    ...
                    success: function(rpta, l, s, o) {
                        button.removeClass("m-loader m-loader--right m-loader--light").attr("disabled", !1)
                            ...
                            if (rpta.status == 200) { 
                           img_update.processQueue();
                            window.location.replace("/Historial/detalle/"+$('#default').val()    );   
                            } else {
    ...
    return {
        init: function() {
            es(), newHistory(), editHistory(), restoreUser(),hideUser(),deleteConfirm(),restoreConfirm(),showUser()
        }
    }
}()
