var propuestaValidate = function() {
    var idForm = "frm_propuesta_editar",
    form = $("#"+idForm),
    vl_finalziar = {},
    vl_guardar = {},
    first = true,
    ...
    participantes = function(){
        $("#btn_crea_participante").click(function(e){
    ...
    guardar = function(){
         $("#btn_propuesta_save_continue").click(function(e) {
            // if(!first)
            //     vl_finalziar.destroy();
                    //btn_propuesta_save_continue
                    e.preventDefault();                
                    var btn = $(this); 
                    var theRules = {
                    ...
                    //vl_guardar = form.validate(theRules);
                    //first = false;
                    form.validate(theRules);
                    form.valid() && (btn.addClass("m-loader m-loader--right m-loader--light").attr("disabled", true), form.ajaxSubmit({
                        ...
                        success: function(rpta, l, s, o) {
                            ...
                            if(rpta != "ok"){
                                    if (rpta == "docs"){
                                        notifyGlobal("warning", "No puedes finalizar con pendientes","Faltan documentos",  "la la-warning")
                                    }else{
                            ...
                            setTimeout(function() {                            
                                if (rpta == "ok"){
                                        notifyGlobal("primary", "Los cambios se realizaron correctamente","Guardado",  "la la-save")
    ...
    return {
        init: function() {
            language(), ciudades(), guardar(), participantes(), firstLoadParticipantes(), triggerModalDelete(), deleteParticipante()
        }
    }
}();

jQuery(document).ready(function() {
    propuestaValidate.init()
});